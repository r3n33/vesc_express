...
static void image_buffer_clear(image_buffer_t *img, uint32_t cc) {
	...
	case indexed16: {
		...
		uint8_t packedByte = (cc4bit << 4) | cc4bit; // Pack two pixels into one byte

		memset(img->data + img->data_offset, packedByte, bytes);
	}
	break;
	case rgb332: {
	...
	if (x < w && y < h) {
		...
		case indexed16: {
			...
			if (nibble == 0) {
				...
				data[byte] = (data[byte] & 0x0F) | (c << 4);
			} else {
				// Modify the low nibble
				data[byte] = (data[byte] & 0xF0) | (c & 0x0F);
			}
			break;
		}
		case rgb332: {
	...
	if (x < w && y < h) {
		...
		case indexed16: {
			...
			if (nibble == 0) {
				...
				return (uint32_t)((data[byte] & 0xF0) >> 4);
			} else {
				// Extract the low nibble
				return (uint32_t)(data[byte] & 0x0F);
			}
		}	
		case rgb332: {
			int pos = y*w + x;
...
static lbm_value ext_disp_load_icna3306(lbm_value *args, lbm_uint argn) {
	...
	disp_render_image = disp_icna3306_render_image;
	disp_clear = disp_icna3306_clear;
	disp_reset = disp_icna3306_reset;
